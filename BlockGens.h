...
std::vector<T> getVaues(const SingleBlock& blockInds, const std::vector<T>& vals) {
	...
	for(SingleBlock::const_iterator it = blockInds.cbegin(); it != blockInds.cend(); ++it)
		tmp.push_back(vals[*it]);
	return tmp;
}


//Adding the functions of get max did and max score within one dimenssion
...
public:
	...
	//One set uses Singleblock, the other uses const BlocksIterator, both to obtain the MaxDid within a block
	//SingleBlock is vecUint, namely std::vector<unsigned int>
...
protected:
	unsigned char bits;
	const vecUInt* docIds;
	std::vector<float> blockIdToMaxScore;  //this is to store the Docid-oriented Block-max score
	//std::vector<unsigned char> QuantizedblockIdToMaxScore;
	inline unsigned int whereDidMapped(unsigned int did) { return did >> bits;}
public:
	static unsigned int expectedBlockSize;
...
*/
	...
	//determine block size according to list length
	static unsigned int bitOracle(unsigned int length) {
		unsigned int lenBits = intlog2(length);
  		std::vector<int> Bucket (18, 0);
  		// Bucket initialization is being done as follows
  		// Parameters
...
*/
	...
	unsigned int getBits() const { return bits; }
	const std::vector<float>& getScores() const { return blockIdToMaxScore; }
	//This is the important function being used
