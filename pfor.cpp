...
unsigned int *pack_decode(unsigned int *_p, unsigned int *_w, int flag,int base) {
  ...
  if (pref > 0)
  {
    ...
    prefix(_p, base);
#else
    _p[0] += base; //(modify version pford decode)adding the last docid in last block(which is also the max docid in last block) to the first docid in this block
    //only use this part
    // printf("base: %d\n", base);
#endif
#if BLOCKDIFF==1
    _p[S-1] += base;
    // printf("BLOCKDIFF==1 base: %d\n", base);
    for (i = 2*S-1; i < CONSTS::BS; i += S)
      _p[i] += _p[i-S];
#endif
  }
  // printf("huh? in pford 4\n");
  //profiler::getInstance().end(CONSTS::DECODE);
  return(_w);
}
